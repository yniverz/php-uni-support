...
/**
 * Persist a per-session override for a requirement's "done" state.
 */
if (!function_exists('markRequirementDone')) {
    function markRequirementDone(string $uuid, bool $done): void
    {
        if (session_status() !== PHP_SESSION_ACTIVE) {
            session_start();
        }
        if (!isset($_SESSION['req_done_overrides'])) {
            $_SESSION['req_done_overrides'] = [];
        }
        $_SESSION['req_done_overrides'][$uuid] = $done ? 1 : 0;
    }
}

/**
 * Get an override for a requirement's "done" state, if set. Returns null if none.
 */
if (!function_exists('getRequirementDoneOverride')) {
    function getRequirementDoneOverride(string $uuid): ?bool
    {
        if (session_status() !== PHP_SESSION_ACTIVE) {
            session_start();
        }
        ...
        return array_key_exists($uuid, $_SESSION['req_done_overrides'])
            ? (bool) $_SESSION['req_done_overrides'][$uuid]
            : null;
    }
}

/**
 * Apply any stored "done" overrides to a flat requirements array.
 * Expects each item to have keys: 'uuid' and 'done'.
 */
if (!function_exists('applyRequirementDoneOverrides')) {
    function applyRequirementDoneOverrides(array $requirements): array
    {
        if (session_status() !== PHP_SESSION_ACTIVE) {
            session_start();
        }
        $ov = $_SESSION['req_done_overrides'] ?? [];
        if (!$ov) {
            return $requirements;
        }

        foreach ($requirements as &$r) {
            if (isset($r['uuid']) && array_key_exists($r['uuid'], $ov)) {
                $r['done'] = (bool) $ov[$r['uuid']];
            }
        }
        unset($r);
